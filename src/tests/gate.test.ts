...
describe("createGate — constructor", () => {
  ...
  it("creates gate with only maxBudget", () => {
    expect(() => createGate({ maxBudget: 0.10 })).not.toThrow()
  })
...
describe("record()", () => {
  ...
  it("accumulates budget from known model pricing", () => {
    const gate = createGate({ maxBudget: 1.00 })
  ...
  it("budget stays 0 for unknown model", () => {
    const gate = createGate({ maxBudget: 1.00 })
    gate.record({ model: "unknown-model-xyz", inputTokens: 1000, outputTokens: 500 })
...
describe("custom pricing", () => {
  it("uses user-injected pricing over defaults", () => {
    ...
    gate.record({ model: "my-custom-model", inputTokens: 100, outputTokens: 50 })
    const status = gate.check()
